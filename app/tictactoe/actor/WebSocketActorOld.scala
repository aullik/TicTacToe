...
/**
  */
class WebSocketActorOld(out: ActorRef, user: User) extends Actor with Logging {
  ...
  def handleAllLoggedInReturn(usrToken: String, userTokenList: List[(String, String)], gameManager: Option[ActorRef]): Unit = {
    ...
    UserStatus(user.name, usrToken, userTokenList.map(ofTuple((usr, token) => UserElement(usr, token))))
  }
...
object WebSocketActorOld {
  ...
  def apply(out: ActorRef, usr: User): Props = {
    Props(new WebSocketActorOld(out, usr))
  }
}